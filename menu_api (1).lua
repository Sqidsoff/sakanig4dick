...
function Library:CreateWindow(env)
	...
	local rootGui = PlayerGui:FindFirstChild("HvH_AllInOne_UI")
	if not rootGui then
		rootGui = mk(PlayerGui, "ScreenGui", {
			Name = "HvH_AllInOne_UI",
			ResetOnSpawn = false,
			IgnoreGuiInset = true,
		})
	end

	if env.oldMenuFrame and env.oldMenuFrame.Parent then
		env.oldMenuFrame.Visible = false
		env.oldMenuFrame.Name = "LegacyMenuHidden"
	end
	...
	local watermark = mk(rootGui, "Frame", {
		...
		Position = env.watermarkPosition or UDim2.new(1, -230, 0, 18),
    ...
    RunService.RenderStepped:Connect(function(dt)
        ...
        if #fpsSamples > 100 then table.remove(fpsSamples, 1) end
        -- approximate 0.1% low fps as the minimum of recent samples
	...
	local navLayout = mk(nav, "UIListLayout", {
	...
	local contentLayout = mk(content, "UIListLayout", {
		SortOrder = Enum.SortOrder.LayoutOrder,
	...
	local bindPopupHint = mk(bindPopup, "TextLabel", {
...
local function selectControl(control, screenPosition)
	...
	function configApi:Snapshot()
		local data = {values = {}, binds = cfg.UIBinds or {}, colorPresets = cfg.ColorPresets or {}}
	...
	function window:Tab(id, name, internal)
		local tab = {id = id, name = name, controls = {}}
		local navText = tostring(name or id)
		if #navText > 1 then navText = navText:sub(1, 1) end
		local navBtn = button(nav, string.upper(navText), UDim2.fromOffset(76, 62))
		navBtn.LayoutOrder = #tabs + 1
		navBtn.Font = uiFontBold
		navBtn.TextSize = textSize(42)
	...
	function api:DisableAllControls()
		for id, control in pairs(controls) do
			if not tostring(id):match("^__aether_") and control.set then
				...
				elseif control.kind == "dropdown" and control.options and control.options[1] ~= nil then
					pcall(function() control.set(control.default or control.options[1]) end)
				elseif control.kind == "list" then
	...
	function window:AddSlider(tab, options)
		...
		corner(valueBox, 0)
		local minLabel = mk(row, "TextLabel", {
		...
		local maxLabel = mk(row, "TextLabel", {
			Size = UDim2.fromOffset(70, 14),
		...
		corner(track, 0)
		stroke(track, Color3.fromRGB(24, 24, 24), 0.2)
	...
	function window:AddDropdown(tab, options)
		...
		local popup = mk(root, "Frame", {
			Size = UDim2.fromOffset(150, math.min(168, math.max(30, #(options.options or {}) * 22 + 6))),
			BackgroundColor3 = theme.root,
			BackgroundTransparency = 0.02,
			BorderSizePixel = 0,
			Visible = false,
		...
		local value = options.default or (options.options and options.options[1]) or ""
		local getter = options.get or function() return value end
		local setter = options.set or function(v) value = v end
		local optionButtons = {}
		local control = {
		...
		function control.refresh()
			pick.Text = tostring(control.get())
			for opt, btn in pairs(optionButtons) do
				local on = tostring(opt) == tostring(control.get())
        ...
        function control.refresh()
			...
			for _, item in ipairs(items) do
				...
				local btn = mk(list, "TextButton", {
					...
					BackgroundColor3 = theme.soft,
					BackgroundTransparency = tostring(control.get()) == itemText and 0.1 or 1,
					BorderSizePixel = 0,
					AutoButtonColor = false,
					Font = uiFont,
					Text = itemText,
					TextSize = textSize(11),
					TextColor3 = tostring(control.get()) == itemText and theme.accent or theme.text,
					TextXAlignment = Enum.TextXAlignment.Left,
					TextTruncate = Enum.TextTruncate.AtEnd,
					ZIndex = 4,
				})
				...
				btn.MouseLeave:Connect(function()
					local on = tostring(control.get()) == itemText
	...
	function window:AddColor(tab, options)
		...
		local preview = button(row, "PICK", UDim2.fromOffset(118, 22))
		preview.Position = UDim2.new(1, -128, 0.5, -11)
		preview.Text = ""
		...
		local swatchGrid = mk(swatchFrame, "UIGridLayout", {
			CellSize = UDim2.fromOffset(22, 14),
			CellPadding = UDim2.fromOffset(6, 6),
			SortOrder = Enum.SortOrder.LayoutOrder,
		})
		local applyBtn = button(picker, "APPLY", UDim2.new(0.33, -14, 0, 24))
		applyBtn.Position = UDim2.new(0, 12, 1, -34)
		applyBtn.ZIndex = 62
		local saveBtn = button(picker, "SAVE", UDim2.new(0.34, -14, 0, 24))
		saveBtn.Position = UDim2.new(0.33, 6, 1, -34)
		saveBtn.ZIndex = 62
		local copyBtn = button(picker, "COPY", UDim2.new(0.33, -14, 0, 24))
		copyBtn.Position = UDim2.new(0.67, 0, 1, -34)
		copyBtn.ZIndex = 62
	...
	ensureSettingsTab = function()
        ...
        -- Section for watermark element toggles
        settings:Section("Watermark")
        -- Show FPS
        settings:Toggle({
            id = "__wm_fps",
            label = "Show FPS",
            default = cfg.WatermarkOptions.fps,
            get = function() return cfg.WatermarkOptions.fps end,
            set = function(v)
                cfg.WatermarkOptions.fps = v
            end,
        })
        -- Show FPS 0.1 low (approx. minimum of recent FPS)
        settings:Toggle({
            id = "__wm_fps01",
            label = "Show FPS 0.1",
            default = cfg.WatermarkOptions.fps01,
            get = function() return cfg.WatermarkOptions.fps01 end,
            set = function(v)
                cfg.WatermarkOptions.fps01 = v
            end,
        })
        ...
        settings:Toggle({
            id = "__wm_nick",
            label = "Show Nickname",
            default = cfg.WatermarkOptions.nick,
            get = function() return cfg.WatermarkOptions.nick end,
            set = function(v)
                cfg.WatermarkOptions.nick = v
            end,
        })
        ...
        settings:Toggle({
            id = "__wm_mode",
            label = "Show Mode",
            default = cfg.WatermarkOptions.mode,
            get = function() return cfg.WatermarkOptions.mode end,
            set = function(v)
                cfg.WatermarkOptions.mode = v
            end,
        })
        -- Show config name
        settings:Toggle({
            id = "__wm_config",
            label = "Show Config",
            default = cfg.WatermarkOptions.config,
            get = function() return cfg.WatermarkOptions.config end,
            set = function(v)
                cfg.WatermarkOptions.config = v
            end,
        })
        -- Show server IP
        settings:Toggle({
            ...
            default = cfg.WatermarkOptions.serverIP,
            get = function() return cfg.WatermarkOptions.serverIP end,
            set = function(v)
                cfg.WatermarkOptions.serverIP = v
            end,
        })
        ...
        settings:Toggle({
            id = "__wm_speed",
            label = "Show Speed",
            default = cfg.WatermarkOptions.speed,
            get = function() return cfg.WatermarkOptions.speed end,
            set = function(v)
                cfg.WatermarkOptions.speed = v
            end,
        })
        -- Show current time
        settings:Toggle({
            id = "__wm_time",
            label = "Show Time",
            default = cfg.WatermarkOptions.time,
            get = function() return cfg.WatermarkOptions.time end,
            set = function(v)
                cfg.WatermarkOptions.time = v
            end,
        })
		settings:Button({
			id = "__aether_unload",
